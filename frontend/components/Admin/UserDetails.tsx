...
function CardMovementModal({
  cardId,
  board,
  isOpen,
  onClose,
}: CardMovementModalProps) {
    ...
    try {
      ...
      for (let i = 0; i < sortedMovements.length; i++) {
        ...
        const listTitle = Object.entries(columnNameMap).find(
          ([_, colName]) => colName === currentMovement.toColumn
        )?.[0];
          ...
          <div className="space-y-4">
              ...
              <TableBody>
                {currentMovements.map((movement, index) => (
                  <TableRow key={index}>
                    ...
                    <TableCell>
                      {displayNameMap[movement.fromColumn] ||
                        movement.fromColumn}
                    </TableCell>
                    <TableCell>
                      {displayNameMap[movement.toColumn] || movement.toColumn}
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
            ...
            <div className="flex items-center justify-between">
              <p className="text-sm text-muted-foreground">
                Showing {startIndex + 1}-{Math.min(endIndex, movements.length)}{" "}
                of {movements.length} movements
              </p>
      ...
      for (const list of board.lists) {
        for (const card of list.cards) {
          try {
            ...
            console.error(
              `Error fetching study time for card ${card.id}:`,
              error
            );
          }
        }
              ...
              {!board ? (
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      This user doesn't have any boards yet
                    </CardDescription>
                  </CardHeader>
                </Card>
              ) : (
