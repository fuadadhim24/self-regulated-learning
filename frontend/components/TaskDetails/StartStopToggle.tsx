...
export default function StartStopToggle({
  cardId,
  listName,
  onToggleStateChange,
}: StartStopToggleProps) {
    ...
    <div className="flex flex-col gap-2">
      <div className="relative group">
        <button
          ...
          disabled={isDisabled}
          className={`relative flex items-center px-4 py-2 rounded-md transition-all duration-200 shadow-sm hover:shadow-md transform hover:-translate-y-0.5 ${
            isDisabled
      ...
      <div className="flex flex-col text-sm text-indigo-600 dark:text-indigo-400">
        ...
        {totalStudyTime > 0 && (
          <div>Total study time: {formatTime(totalStudyTime)}</div>
        )}
      </div>
    </div>
  );
