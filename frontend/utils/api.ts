...
export async function searchBoards(query: string) {
  return authorizedFetch(
    `${API_URL}/search-boards?q=${encodeURIComponent(query)}`
  );
}
...
export const addLearningStrategy = async (
  strategy: LearningStrategyInput
): Promise<Response> => {
  return authorizedFetch(`${API_URL}/learningstrats`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export const updateLearningStrategy = async (
  id: string,
  strategy: LearningStrategyInput
): Promise<Response> => {
  return authorizedFetch(`${API_URL}/learningstrats/${id}`, {
